...
> **最新版本**: v0.3.0+ (2026-02-10)
> **项目状态**: 活跃开发中，支持 Qdrant 向量数据库、自定义 Agent 系统、Helm Chart 部署、数据分析师 Agent、Google 搜索、GPUStack 等多平台模型
>
> **变更记录 (Changelog)**
> - 2026-02-10: 增量更新，同步最新提交（协作知识库 UI 增强、剪贴板 API 回退、DuckDB 扩展修复、Presigned URL 加载知识文件等）
...
## v0.3.0+ 最新更新 (2026-02-10)

### 🚀 协作功能增强
- **共享知识库 UI 增强**: ListSpaceSidebar 支持显示共享知识库
- **Agent 编辑器分类**: 将知识库分类为"我的知识库"和"协作知识库"
- **KnowledgeBaseList 共享资源**: 知识库列表新增共享知识库区域

### 🌐 国际化与兼容性
- **剪贴板 API 回退**: 为非安全上下文（HTTPS）添加剪贴板 API 兼容性支持
- **多语言支持**: 新增共享知识库相关的中英文翻译

### 🐛 问题修复
- **DuckDB 扩展**: 修复 spatial extension 未找到错误
- **数据分析师 Agent**: 通过 presigned URLs 安全加载知识文件
- **兼容性增强**: 提升非 HTTPS 环境下的组件可用性
...
### 2026-02-10: v0.3.0+ 增量更新
- 同步最新 git 提交记录
- 新增协作知识库 UI 增强（ListSpaceSidebar 共享资源显示）
- 新增 Agent 编辑器知识库分类功能
- 新增剪贴板 API 非安全上下文回退支持
- 修复 DuckDB spatial extension 未找到错误
- 修复数据分析师 Agent 通过 presigned URLs 加载知识文件
- 更新版本号至 v0.3.0+
